...
class RegisterUserUseCaseTest {
  ...
  private final UUID actorId = UUID.randomUUID();
   private final Role admin = new Role(UUID.randomUUID(), "ADMIN");
  ...
  void setUp() {
    ...
    useCase = new RegisterUserUseCase(userRepository, roleRepository, passwordEncoder);
    user =
        User.builder()
            .name("name")
            .lastName("lastName")
  ...
  void shouldPassValidationWithValidUser() {
    when(roleRepository.findUserRole(actorId)).thenReturn(Mono.just(admin));
    when(userRepository.existsByEmail("email@email.com")).thenReturn(Mono.just(false));
    when(roleRepository.findDefaultRole()).thenReturn(Mono.just(admin));
    when(userRepository.saveUser(any(User.class))).thenReturn(Mono.just(user));
    when(userRepository.saveUser(any(User.class))).thenReturn(Mono.just(user));
    StepVerifier.create(useCase.execute(user, actorId))
        .assertNext(
            u -> {
